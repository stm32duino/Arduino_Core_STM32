...
/*        - ADC clock from asynchronous clock with prescaler 512,             */
/*          with highest ratio CPU clock frequency vs HSI clock frequency:    */
/*          CPU clock frequency max 100MHz, HSI frequency 16MHz: ratio 7.     */
/* Unit: CPU cycles.                                                          */
#define ADC_CLOCK_RATIO_VS_CPU_HIGHEST          (512UL * 7UL)
...
ErrorStatus LL_ADC_CommonDeInit(const ADC_Common_TypeDef *ADCxy_COMMON)
{
  ...
  SET_BIT(RCC->AHB4RSTR, RCC_AHB4ENR_ADC4EN);

  /* Release reset of ADC clock (core clock) */
  CLEAR_BIT(RCC->AHB4RSTR, RCC_AHB4ENR_ADC4EN);
      ...
      == 0UL)
  {
    ...
    CLEAR_BIT(ADCx->CFGR1,
              (ADC_CFGR1_AWD1CH  | ADC_CFGR1_AWD1EN | ADC_CFGR1_AWD1SGL | ADC_CFGR1_DISCEN
               | ADC_CFGR1_CHSELRMOD | ADC_CFGR1_WAIT   | ADC_CFGR1_CONT    | ADC_CFGR1_OVRMOD
    ...
    MODIFY_REG(ADCx->AWD2TR, ADC_AWD2TR_LT2 | ADC_AWD2TR_HT2, ADC_AWD2TR_HT2);
