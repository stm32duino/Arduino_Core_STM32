...
{
  size_t ret = size;
  // If begin() never brought the hardware up (e.g. the requested baud
  // rate can't be reached from the configured kernel clock), the TX
  // interrupt this function waits on below will never fire. Bail out
  // instead of spinning forever once the TX buffer fills up.
  if (!_ready) {
    ret = 0;
  } else {
  ...
  /* Return 0 if not ready else requested transfer size */
  return ret;
}
