...
USBD_CDC_ItfTypeDef USBD_CDC_fops = {USBD_CDC_Init, USBD_CDC_DeInit,
...
/**
  ...
  *         USBD_FAIL
  */
static int8_t USBD_CDC_DeInit(void) { return (USBD_OK); }
...
static int8_t USBD_CDC_Control(uint8_t cmd, uint8_t *pbuf, uint16_t length)
{
  ...
  *****************************************************************************/
  ...
  case CDC_SET_CONTROL_LINE_STATE:
    // Check DTR state
    lineState = (uint32_t)((((USBD_SetupReqTypedef *)pbuf)->wValue & 0x01) != 0);
    break;
  ...
  if (!CDC_initialized)
  {
    ...
    if (USBD_Init(&hUSBD_Device_CDC, &CDC_Desc, 0) == USBD_OK)
    {
      ...
      if (USBD_RegisterClass(&hUSBD_Device_CDC, USBD_CDC_CLASS) == USBD_OK)
      {
        ...
        if (USBD_CDC_RegisterInterface(&hUSBD_Device_CDC, &USBD_CDC_fops) ==
            USBD_OK)
        {
...
void CDC_continue_transmit(void)
{
  ...
  USBD_CDC_HandleTypeDef *hcdc =
      (USBD_CDC_HandleTypeDef *)hUSBD_Device_CDC.pClassData;
  /*
