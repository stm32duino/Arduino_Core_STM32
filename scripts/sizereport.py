...
parser.add_argument("-x", "--tool", type=pathlib.Path, required=True, help="path to `arm-none-eabi-size`")
parser.add_argument("-f", "--binfile", type=pathlib.Path, required=True, help="path to the binary to analyze")
parser.add_argument("--progmem", type=int, required=True, help="max amount of program storage space")
parser.add_argument("--datamem", type=int, required=True, help="max amount of dynamic memory")

shargs = parser.parse_args()


proc = subprocess.run((shargs.tool, "-A", shargs.binfile), capture_output=True, encoding="UTF-8")
